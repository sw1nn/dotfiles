;;; st.el --- define function key sequences and standard colors for st
...
(defvar st-function-map
  (let ((map (make-sparse-keymap)))
    ...
    ;; (define-key st-function-map "\e[23~" [f11])
    ;; (define-key st-function-map "\e[24~" [f12])
    ...
    (define-key map "\e[1;8A" [C-S-up])
    (define-key map "\e[1;8B" [C-S-down])
    (define-key map "\e[1;8C" [C-S-right])
    (define-key map "\e[1;8D" [C-S-left])

    (define-key map "\e[1;9A" [S-M-up])
    (define-key map "\e[1;9B" [S-M-down])
    (define-key map "\e[1;9C" [S-M-right])
    (define-key map "\e[1;9D" [S-M-left])

    map)
  "Function key overrides for st.")

(defvar st-alternatives-map
  (let ((map (make-sparse-keymap)))
...
(defun terminal-init-st ()
  ...
  (let ((map (copy-keymap st-alternatives-map)))
  ...
  (let ((m (copy-keymap st-function-map)))
    (set-keymap-parent m (keymap-parent input-decode-map))
    (set-keymap-parent input-decode-map m))

  ;; Initialize colors and background mode.
  (st-register-default-colors)
  (st-set-background-mode)
  ;; This recomputes all the default faces given the colors we've just set up.
  (tty-set-up-initial-frame-faces))

;; Set up colors, for those versions of st that support it.
(defvar st-standard-colors
  ;; The names of the colors in the comments taken from the st.1 man
  ...
  "Names of 16 standard st colors, their numbers, and RGB values.")

(defun st-rgb-convert-to-16bit (prim)
  "Convert an 8-bit primary color value PRIM to a corresponding 16-bit value."
  (min 65535 (round (* (/ prim 255.0) 65535.0))))

(defun st-register-default-colors ()
  "Register the default set of colors for st or compatible emulator.

This function registers the number of colors returned by `display-color-cells'
for the currently selected frame."
  (let* ((ncolors (display-color-cells))
         (colors st-standard-colors)
         (color (car colors)))
    (if (> ncolors 0)
    ...
    (while (and (> ncolors 0) colors)
      (tty-color-define (car color) (cadr color)
                        (mapcar 'st-rgb-convert-to-16bit
                                (car (cddr color))))
    ...
    (when (> ncolors 0)
      (cond
       ((= ncolors 240)                 ; 256-color st
        ...
        (while (> ncolors 0)
          (setq color (st-rgb-convert-to-16bit (+ 8 (* (- 24 ncolors) 10))))
          (tty-color-define (format "color-%d" (- 256 ncolors))
                            (- 256 ncolors)
                            (list color color color))
          (setq ncolors (1- ncolors))))

       ((= ncolors 72) ; st-unicode
        ;; 64 non-gray colors
        ...
        (while (> ncolors 0)
          (setq color (st-rgb-convert-to-16bit
                       (floor
                        (if (= ncolors 8)
                            46.36363636
          ...
          (tty-color-define (format "color-%d" (- 88 ncolors))
                            ...
                            (list color color color))
          (setq ncolors (1- ncolors))))
       (t (error "Unsupported number of st colors (%d)" (+ 16 ncolors)))))
    ;; Modifying color mappings means realized faces don't use the
    ;; right colors, so clear them.
    (clear-face-cache)))

;; st puts the default colors into an environment variable
;; COLORFGBG.  We use this to set the background mode in a more
;; intelligent way than the default guesswork in startup.el.
(defun st-set-background-mode ()
  "Set background mode as appropriate for the default st colors."
  (let ((fgbg (getenv "COLORFGBG"))
    ...
    (when (and fgbg
      ...
      ;; The next line assumes that st-standard-colors are ordered
      ;; by the color index in the ascending order!
      (setq rgb (car (cddr (nth bg st-standard-colors))))
      ;; See the commentary in frame-set-background-mode about the
      ;; computation below.
      (if (< (apply '+ rgb)
             ;; The following line assumes that white is the 15th
             ;; color in st-standard-colors.
             (* (apply '+ (car (cddr (nth 15 st-standard-colors)))) 0.6))
